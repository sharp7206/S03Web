...
function initPage(){
	  $('ul.tabs li').click(function(){
		    ...
		    if(tab_id =='tab-1' ){
		    	doAction('SearchMaker');
		    }else if(tab_id =='tab-2' ){
		    	doAction('SearchGroup');
	...
	APPz.cmn.ApiRequest("${pageContext.request.contextPath}/api/cmn/common/getCodeList.do"
		...
		, function($result){
			...
			_expireYmd = new AirDatepicker('#EXPIRE_YMD',{
				autoClose:true
		    });
			_loanYmd = new AirDatepicker('#LOAN_YMD',{
				autoClose:true
		    });
...
function fn_fileView(fileListJson){//fileView_div
    ...
    fileListJson.forEach(function(fileInfo) {
        ...
        //fileDiv.textContent = "<DIV>""File Name: " + fileInfo.FILE_ORG_NM + ", File Size: " + fileInfo.FILE_SZ + ", File Type: " + fileInfo.FILE_TYPE;
        fileDiv.textContent = "<DIV><a href=\"javascript:APPz.filedownN('"+fileInfo.FILE_PATH+"', '"+fileInfo.FILE_ORG_NM+"')\"></DIV>";
...
<div class="container-fluid mt-2 h-100">
		...
		<div class="table-type1">
			<table>
				...
				<tbody>
					...
					<tr>
						<th>월이자율<span aria-label="필수입력"></span></th>
						<td>
							<input type="text" name="INTEREST_RATE" id="INTEREST_RATE"  required placeholder="월이자율" style="width:100px; text-align: right;" onkeyup='APPz.ui.floatInput(event, $("INTEREST_RATE"), 2)' onchange="javascript:calcInterest()">부 
  ...
  <div id="tab-1" class="tab-content current">				
        ...
        <div class="layout-type1 mt20">
            <div>
				<script>
				createIBSheet("mySheet1", "100%", $(window).height()-500+"px");
				</script>
            </div>
