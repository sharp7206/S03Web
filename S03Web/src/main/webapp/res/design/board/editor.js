...
this.imgURL = _editor_url + 'img/editor/';
this.debug  = 0;
...
//    ['OrderedList','UnOrderedList','Outdent','Indent', 'separator'],
    ['forecolor','backcolor'],
//	['HorizontalRule','separator','Createlink','InsertImage','inserttable','htmlmode']];
	['HorizontalRule','separator','Createlink','InsertImage','htmlmode']];
...
this.btnList = {
    ...
    "italic":         ['Italic',               '이탤릭',             'editor_action(this.id)',  'ed_format_italic.gif'],
    "underline":      ['Underline',            '밑줄',          'editor_action(this.id)',  'ed_format_underline.gif'],
    "strikethrough":  ['StrikeThrough',        '취소선',      'editor_action(this.id)',  'ed_format_strike.gif'],
    "subscript":      ['SubScript',            '아래 첨자',          'editor_action(this.id)',  'ed_format_sub.gif'],
    "superscript":    ['SuperScript',          '위 첨자',        'editor_action(this.id)',  'ed_format_sup.gif'],
    ...
    "justifyright":   ['JustifyRight',         '오늘쪽 정렬',      'editor_action(this.id)',  'ed_align_right.gif'],
    "orderedlist":    ['InsertOrderedList',    '순서 목록',       'editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',  '순서없는 목록',      'editor_action(this.id)',  'ed_list_bullet.gif'],
    "outdent":        ['Outdent',              '내어쓰기',    'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               '들여쓰기',    'editor_action(this.id)',  'ed_indent_more.gif'],
    "forecolor":      ['ForeColor',            '글씨색',         'editor_action(this.id)',  'ed_color_fg.gif'],
    "backcolor":      ['BackColor',            '배경색',   'editor_action(this.id)',  'ed_color_bg.gif'],
    "horizontalrule": ['InsertHorizontalRule', '라인넣기',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           'URL삽입',    'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          '이미지링크',       'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          '표만들기',       'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'HTML소스보기',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '창 크게 보기',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
...
function editor_action(button_id) {
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog("../../html/popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
      ...
      showModalDialog("../../page/f1/ZBA?crud=R_EditUpload", editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog("/s03/html/popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
