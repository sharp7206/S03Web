...
function createIBSheet(sheetid, width, height, locale) {
    ...
    if (!locale) locale = "";
    if ('ko'==locale) locale = "kr";
    Grids.Locale = locale;

    div_str += "<div name='DIV_IBSheet' id='DIV_" + sheetid + "' style='width:" + width + ";height:" + height + ";'>";
...
function sortEventOverRide (id){  
    var dummy2 = null;
      //OnSort이벤트가 사용중인지 확인
      if(typeof(window[id+"_OnSort"])!="undefined"){  
        if(typeof window[id+"_ OnSort"] == "function"){
            dummy2 = window[id+"_ OnSort"];
        }
      }
      window[id+"_OnSort"] = function(Col, SortOrder){
      // 소팅전 처리할 내용
      window[id].SetSelectRow(-1);
        if(dummy2){
            dummy2(Col, SortOrder);  //화면에 정의한 OnSort 이벤트가 있으면 이것도 수행함.
        }
    }
}  
...
/**
 * sso에 의한 IBSheet 조회 처리 (제품조회 팝업 조회시) Row값을 인자로 넘길때 처리
 */
function getDoSearchMethodRow(sheet_Obj, fileNm, params, row, chk) {
	var sXml = sheet_Obj.GetSearchXml(fileNm, params);
	sheet_Obj.LoadSearchXml(sXml, chk, row);
	/*
	var aryXml = sXml.split("|!|");
	if (0 < aryXml.length) {
		sheet_Obj.LoadSearchXml(aryXml[1], chk, row);
	} else {
		sheet_Obj.LoadSearchXml(aryXml[0], chk, row);
	}
	*/
}